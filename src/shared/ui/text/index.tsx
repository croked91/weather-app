...
const UnStyledText: FC<IText> = ({tag, children, className}) => {
    return (
        <>
            {tag === 'h1' && (
                <h1 className={['h1', `${className}`].join(' ')}>{children}</h1>
            )}
            {tag === 'h2' && (
                <h2 className={['h2', `${className}`].join(' ')}>{children}</h2>
            )}
            {tag === 'h3' && (
                <h3 className={['h3', `${className}`].join(' ')}>{children}</h3>
            )}
            {tag === 'h4' && (
                <h4 className={['h4', `${className}`].join(' ')}>{children}</h4>
            )}
            {tag === 'h5' && (
                <h5 className={['h5', `${className}`].join(' ')}>{children}</h5>
            )}
            {tag === 'h6' && (
                <h6 className={['h6', `${className}`].join(' ')}>{children}</h6>
            )}
            {tag === 'p' && (
                <p className={['p', `${className}`].join(' ')}>{children}</p>
            )}
        </>
    );
};
