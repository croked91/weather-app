...
export const getCityWeatherFX = createEffect(async (payload: {
...
export const $citiesWeather = createStore<Weather[]>(citiesWeatherLocalStorage.init([]))
    ...
    .on(reloadCityWeatherFX.doneData, (state, res) => res && res.map(el => {
        return { ...el, name: state.filter(city => city.id === el.id)[0].name }
    }))
...
const { tick } = interval({ timeout: 60000, start: startReloadTrigger, stop: stopReloadTrigger })
...
sample({
    clock: Gate.state,
    filter: (state) => !JSON.parse(localStorage.getItem('citiesWeather') || '{}').length && state.data.geo_lat == "",
    fn: () => DEFAULT_CITY,
    target: getCityWeatherFX
})